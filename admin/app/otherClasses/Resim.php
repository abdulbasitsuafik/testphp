...
class Resim {
     ...
     *
     * @param string        $overlay        An image filename or a Resim object
    ...
    public function overlay($overlay, $position = 'center', $opacity = 1, $x_offset = 0, $y_offset = 0) {
        ...
        // Load overlay image
        if( !($overlay instanceof Resim) ) {
            $overlay = new Resim($overlay);
        }
     ...
     *
     */
    public function text($text, $font_file, $font_size = 12, $color = '#000000', $position = 'center', $x_offset = 0, $y_offset = 0, $stroke_color = null, $stroke_size = null, $alignment = null, $letter_spacing = 0) {
     ...
     * Thumbnail
     *
     * This public function attempts to get the image to as close to the provided dimensions as possible, and then crops the
    ...
    public function resimHologram($telifresim){
        $jpg = imagecreatefromjpeg($telifresim["jpg"]);          
        $ix = imagesx($jpg);
        $iy = imagesy($jpg); 
        if($telifresim["ayarlar"]["telif_resmi_secim"]=="resim"){             
            $png = imagecreatefrompng($telifresim["telif_resmi"]);
            $sx = imagesx($png);
            $sy = imagesy($png);  
        }                
        if($telifresim["ayarlar"]["telif_resmi_secim"]=="yazi"){   
            if($telifresim["ayarlar"]["telif_resmi_yazi_font"]==""){                
                $font = "../public/eklentiler/font/Blurmix_0.TTF";
            }else{
                $font = "../".$telifresim["ayarlar"]["telif_resmi_yazi_font"];
            }
            if($telifresim["ayarlar"]["telif_resmi_yazi_font_size"]==""){                
                $fontsize = 30;
            }else{
                $fontsize = $telifresim["ayarlar"]["telif_resmi_yazi_font_size"];
            }
            if($telifresim["ayarlar"]["telif_resmi_yazi"]==""){                
                $yazi = "yazı alanını doldurunuz";
            }else{
                $yazi = $telifresim["ayarlar"]["telif_resmi_yazi"];
            }
            list($br, $bg, $bb) = sscanf($telifresim["ayarlar"]["telif_resmi_yazi_bg"], "#%02x%02x%02x");
            list($yr, $yg, $yb) = sscanf($telifresim["ayarlar"]["telif_resmi_yazi_renk"], "#%02x%02x%02x");
            $png=imagecreatetruecolor($telifresim["ayarlar"]["telif_resmi_yazi_width"],$telifresim["ayarlar"]["telif_resmi_yazi_height"]);
            imagesavealpha($png, true);
            $color = imagecolorallocatealpha($png, 0, 0, 0, 127);
            imagefill($png, 0, 0, $color);
            //header('Content-type: image/png');
            $text_renk = imagecolorallocate($png, $yr, $yg, $yb);
            $bg1 = imagecolorallocate($png, $br, $bg, $bb);
            $sx = imagesx($png);
            $sy = imagesy($png); 
            imagettftext($png, $fontsize,0, 60,$sy / 2, $text_renk, $font, $yazi);
            //imagepng($png);
            //imagedestroy($png);
                         
        }  
        if($telifresim["ayarlar"]["telif_resmi_konum"]=="orta"){            
            $marge_right = ($ix / 2) - ($sx / 2);
            $marge_bottom = ($iy / 2) - ($sy / 2);  
        }else if($telifresim["ayarlar"]["telif_resmi_konum"]=="solust"){//tamam
            $marge_right = 0;
            $marge_bottom = 0; 
        }else if($telifresim["ayarlar"]["telif_resmi_konum"]=="ortaust"){//tamam
            $marge_right = ($ix / 2) - ($sx / 2);
            $marge_bottom = 0; 
        }else if($telifresim["ayarlar"]["telif_resmi_konum"]=="sagust"){//tamam
            $marge_right = ($ix) - ($sx);
            $marge_bottom = 0; 
        }else if($telifresim["ayarlar"]["telif_resmi_konum"]=="solorta"){//tamam
            $marge_right = 0 ;
            $marge_bottom = ($iy / 2) - ($sy / 2); 
        }else if($telifresim["ayarlar"]["telif_resmi_konum"]=="sagorta"){//tamam
            $marge_right = ($ix) - ($sx);
            $marge_bottom = ($iy / 2) - ($sy / 2); 
        }else if($telifresim["ayarlar"]["telif_resmi_konum"]=="solalt"){//tamam
            $marge_right = 0;
            $marge_bottom = ($iy) - ($sy); 
        }else if($telifresim["ayarlar"]["telif_resmi_konum"]=="ortaalt"){
            $marge_right = ($ix / 2) - ($sx / 2);
            $marge_bottom = ($iy) - ($sy); 
        }else if($telifresim["ayarlar"]["telif_resmi_konum"]=="sagalt"){
            $marge_right = ($ix) - ($sx);
            $marge_bottom = ($iy) - ($sy); 
        } 
        imagecopy($jpg, $png, $marge_right, $marge_bottom, 0, 0, $sx, $sy);
            header('Content-type: image/jpeg');
            imagejpeg($jpg,$telifresim["yeniResim"]);
            imagedestroy($jpg);
    }
