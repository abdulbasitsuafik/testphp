...
class devices_model extends Model{
    ...
    public function devices_list($data=null){
        $sql = "SELECT * ";
        $sql.=" FROM ".PREFIX."devices WHERE id <> :id ";
        $sql.= $data;
    ...
    public function get_list() {
        $sql = "SELECT * FROM ".PREFIX."devices order by id DESC";
        return $this->db->select($sql);
    }
    ...
    public function get_list_user_order_point() {
        $sql = "SELECT * FROM ".PREFIX."devices order by point DESC";
        return $this->db->select($sql);
    }
    public function update_just_point($data)
    {
        ...
        $this->db->update(PREFIX."devices", $postData, "`id` = '{$data["user"]}'");
    }
    public function get_single($id)
    {
        $sql = "SELECT * FROM ".PREFIX."devices WHERE id = :id";
        return $this->db->select($sql, array(':id' => $id));
    }
    public function get_single_devices($id)
    {
        $sql = "SELECT * FROM ".PREFIX."devices WHERE id = :id";
        return $this->db->select($sql, array(':id' => $id));
    }
...
//        $data["updated_at"] = strtotime("now");
       return $this->db->insert(PREFIX."devices", $data);
    }
    public function add_devices_points_monthly($data){
        $d = new DateTime();
        $created_at = $d->format('Y-m-d H:i:s');
        $data["created_at"] = $created_at;
       return $this->db->insert(PREFIX."devices_points_monthly", $data);
    }
    public function get_devices_points_monthly($data)
    {
        $sql = "SELECT * FROM ".PREFIX."devices_points_monthly WHERE year = :year AND month = :month ";
        return $this->db->select($sql,[":month"=>$data["month"],":year"=>$data["year"]]);
    }
    public function get_devices_sessions($id)
    {
    ...
    {
        $sql = "SELECT * FROM ".PREFIX."`devices_permissions_role` order by id ASC ";
        return $this->db->select($sql);
    }
    ...
    {
        $sql = "SELECT a.*,b.name AS product_name FROM ".PREFIX."devices a JOIN products b ON a.product = b.id  WHERE a.user = :user AND a.payment = :payment  ";
    ...
    public function get_devices($data)
    {
        ...
        $sql = "SELECT * FROM ".PREFIX."devices WHERE  ".$add_sql;
        return $this->db->select($sql,$array);
    }
...
//        $data["updated_at"] = strtotime("now");
        ...
        return $this->db->update(PREFIX."devices", $postData, "`id` = '{$data['id']}'");
    }
    public function delete($id)
    {
        $this->db->delete(PREFIX."devices", "`id` = '$id'");
    }
    public function change_status($id,$status)
    {
        ...
        $this->db->update(PREFIX."devices", $postData, "`id` = '{$id}'");
        return $status;
    }
    public function devices_status($id,$status)
    {
        if($status==1){$status=0;}else{$status=1;}
        $postData = array(
            'used' => $status
    ...
    public function email_control($email) {
        $sql = "SELECT * FROM ".PREFIX."devices WHERE email= :email";
