...
class users_model extends Model{
    ...
    public function users_list($data=null){
        $sql = "SELECT * ";
        $sql.=" FROM ".PREFIX."users WHERE id <> :id ";
        $sql.= $data;
    ...
    public function get_list() {
        $sql = "SELECT * FROM ".PREFIX."users order by id DESC";
        return $this->db->select($sql);
    }
    ...
    public function get_list_user_order_point() {
        $sql = "SELECT * FROM ".PREFIX."users order by point DESC";
        return $this->db->select($sql);
    }
    public function update_just_point($data)
    {
        ...
        $this->db->update(PREFIX."users", $postData, "`id` = '{$data["user"]}'");
    }
...
//        $data["updated_at"] = strtotime("now");
       return $this->db->insert(PREFIX."users", $data);
    }
    public function add_users_points_monthly($data){
        $d = new DateTime();
        $created_at = $d->format('Y-m-d H:i:s');
        $data["created_at"] = $created_at;
       return $this->db->insert(PREFIX."users_points_monthly", $data);
    }
    public function get_users_points_monthly($data)
    {
        $sql = "SELECT * FROM ".PREFIX."users_points_monthly WHERE year = :year AND month = :month ";
        return $this->db->select($sql,[":month"=>$data["month"],":year"=>$data["year"]]);
    }
    public function get_users_sessions($id)
    {
    ...
    {
        $sql = "SELECT * FROM ".PREFIX."`users_permissions_role` order by id ASC ";
        return $this->db->select($sql);
    }
...
//        $data["updated_at"] = strtotime("now");
        ...
        if(@$data['my_ref_code']!='') {
            $postData["my_ref_code"] = @$data['my_ref_code'];
        }
        if(@$data['ads_enabled']!='') {
        ...
        return $this->db->update(PREFIX."users", $postData, "`id` = '{$data['id']}'");
    }
    public function delete($id)
    {
        $this->db->delete(PREFIX."users", "`id` = '$id'");
        ...
        $this->db->delete(PREFIX."users_notifications", "`user_id` = '$id'");
    }
    public function change_status($id,$status)
    {
        ...
        $this->db->update(PREFIX."users", $postData, "`id` = '{$id}'");
        return $status;
    }
