...
class drives_model extends Model{
    ...
    public function drives_list($data=null){
        $sql = "SELECT * ";
        $sql.=" FROM ".PREFIX."mydrives WHERE id <> :id ";
        $sql.= $data;
    ...
    public function get_list() {
        $sql = "SELECT * FROM ".PREFIX."mydrives order by id DESC";
        return $this->db->select($sql);
    }
    ...
    public function get_list_user_order_point() {
        $sql = "SELECT * FROM ".PREFIX."mydrives order by point DESC";
        return $this->db->select($sql);
    }
    public function update_just_point($data)
    {
        ...
        $this->db->update(PREFIX."mydrives", $postData, "`id` = '{$data["user"]}'");
    }
    public function get_single($id)
    {
        $sql = "SELECT * FROM ".PREFIX."mydrives WHERE id = :id";
        return $this->db->select($sql, array(':id' => $id));
    }
    public function get_single_drives($id)
    {
        $sql = "SELECT * FROM ".PREFIX."mydrives WHERE id = :id";
        return $this->db->select($sql, array(':id' => $id));
    }
...
//        $data["updated_at"] = strtotime("now");
       return $this->db->insert(PREFIX."mydrives", $data);
    }
    public function add_mydrives_points_monthly($data){
        $d = new DateTime();
        $created_at = $d->format('Y-m-d H:i:s');
        $data["created_at"] = $created_at;
       return $this->db->insert(PREFIX."mydrives_points_monthly", $data);
    }
    public function get_mydrives_points_monthly($data)
    {
        $sql = "SELECT * FROM ".PREFIX."mydrives_points_monthly WHERE year = :year AND month = :month ";
        return $this->db->select($sql,[":month"=>$data["month"],":year"=>$data["year"]]);
    }
    public function get_mydrives_sessions($id)
    {
    ...
    {
        $sql = "SELECT * FROM ".PREFIX."`mydrives_permissions_role` order by id ASC ";
        return $this->db->select($sql);
    }
    ...
    public function get_user_mydrives($id)
    {
        $sql = "SELECT a.*,b.name AS product_name FROM ".PREFIX."mydrives a JOIN products b ON a.product = b.id  WHERE a.user = :user AND a.payment = :payment  ";
        return $this->db->select($sql,[":user"=>$id,":payment"=>1]);
    }
    public function get_mydrives($data)
    {
        $add_sql = "";
        $array = [];
        if (@$data["id"]){
        ...
        $sql = "SELECT * FROM ".PREFIX."mydrives WHERE  ".$add_sql;
        return $this->db->select($sql,$array);
    }
...
//        $data["updated_at"] = strtotime("now");
        ...
        return $this->db->update(PREFIX."mydrives", $postData, "`id` = '{$data['id']}'");
    }
    public function delete($id)
    {
        $this->db->delete(PREFIX."mydrives", "`id` = '$id'");
    }
    public function change_status($id,$status)
    {
        ...
        $this->db->update(PREFIX."mydrives", $postData, "`id` = '{$id}'");
        return $status;
    }
    public function drives_status($id,$status)
    {
        if($status==1){$status=0;}else{$status=1;}
        $postData = array(
            'used' => $status
    ...
    public function email_control($email) {
        $sql = "SELECT * FROM ".PREFIX."mydrives WHERE email= :email";
