...
class Devices extends Controller{
    public function __construct() {
        ...
        $this->newmodel = $this->load->model("devices_model");
...
//                "premium_competition" => $form->values['premium_competition'],
            ...
            if(@$_FILES){
                ...
                if (is_dir("upload/devices" )) {
                } else {
                    mkdir( "upload/devices");
                }
                $output_dir = "upload/devices/";
            ...
            if($form->values["premium_competition"]==1){
                ...
                $add_to_devices = $this->newmodel->add_to_cart($add_to_cart_data);
            }
            if($form->values["key_competition"]==1){
                ...
                if ($key_adet){
                    for ($i=0;$i < $key_adet;$i++){
                        $add_to_devices = $this->newmodel->add_to_cart($add_to_cart_key_data);
                    }
                }else{
                    $add_to_devices = $this->newmodel->add_to_cart($add_to_cart_key_data);
                }
            ...
            if($form->values["ticket_competition"]==1){
                ...
                if ($key_adet){
                    for ($i=0;$i < $key_adet;$i++){
                        $add_to_devices = $this->newmodel->add_to_cart($add_to_cart_ticket_data);
                    }
                }else{
                    $add_to_devices = $this->newmodel->add_to_cart($add_to_cart_ticket_data);
                }
    ...
    public function add() {
        ...
        if(@$_POST["modal_getir"]){
            ...
            $data = [];
            ob_start();
            $this->view->render("devices/modal",$data,"1");
    ...
    public function edit() {
        ...
        if(@$_POST["modal_getir"]){
            ...
            $data["premium"] = $model->get_devices(["user"=>$data["id"],"payment"=>1,"used"=>0,"product"=>1]) ? 1 : 0;
            ...
            $data["cozdugu_testler"] = $this->newmodel->get_devices_sessions($form->values["id"]);

            $get_user_devices = $model->get_user_devices($data["id"]);

            $data["get_user_devices"] = $get_user_devices;
...
//            $devices_list = $model->devices_list();
//            foreach ($devices_list as $index => $item) {
...
//            }
    ...
    public function used_status(){
        ...
        if ($_POST){
            ...
            $get_single = $model->get_single_devices($id);
            $status = $model->devices_status($id,$get_single[0]["used"]);
            $response["status"] = $status;
            echo json_encode($response);
        }
    }
...
//                $sql.=" AND end_date = '".$requestData['columns'][5]['search']['value']."' ";
        ...
        if( !empty($requestData['columns'][8]['search']['value']) ){  //salary
            $sql.=" AND runned = '".($requestData['columns'][8]['search']['value'])."' ";
        }
        if( !empty($requestData['columns'][9]['search']['value']) ){  //salary
            ...
            $makale_liste = $model->devices_list($sql);
            $toplammakale = $model->devices_list();
          //echo $sql;
            $durum = "";
            $data = array();
            foreach ($makale_liste as $key => $row) {
                    ...
                    // preparing an array
                       ...
                       $nestedData[] = $row["id"];
                       $nestedData[] = $row["user"];
                    $nestedData[] = $row["device_id"]."---".$row["dark_mode"]."---".$row["notification"];
                       $nestedData[] = $row["app_version"];
                       $nestedData[] = $row["access_token"];
                       $nestedData[] = $row["os_type"];
                        $nestedData[] = $row["brand"];
                        $nestedData[] = $row["model"];
                       $nestedData[] = $row["os_version"];
                       $nestedData[] = $row["local_language"];
